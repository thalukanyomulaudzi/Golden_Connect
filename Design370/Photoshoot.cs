...
namespace Design370
{
    class Photoshoot
    {
        ...
        public static void SearchPhotoshootPackage (System.Windows.Forms.DataGridView dgv)
        {
            try
            {
                DBConnection dBConnection = DBConnection.Instance();
        ...
        public static void DeletePhotoshoot(System.Windows.Forms.DataGridView dgv)
        {
            try
            {
                DBConnection dBConnection = DBConnection.Instance();
                if (dBConnection.IsConnect())
                {
                    string query = "DELETE FROM `booking_package_product` WHERE booking_package_id = '" + GetRowPhotoshoot + "'";
                    var command = new MySqlCommand(query, dBConnection.Connection);
                    command.ExecuteNonQuery();
                    query = "DELETE FROM `booking_package_service` WHERE booking_package_id = '" + GetRowPhotoshoot + "'";
                    command = new MySqlCommand(query, dBConnection.Connection);
                    command.ExecuteNonQuery();
                    query = "DELETE FROM `booking_package` WHERE booking_package_id = '" + GetRowPhotoshoot + "'";
                    command = new MySqlCommand(query, dBConnection.Connection);
                    command.ExecuteNonQuery();
            ...
            {
                System.Windows.Forms.MessageBox.Show("This package is being used, and therefore cannot be deleted");
            }
        }
    }
}
